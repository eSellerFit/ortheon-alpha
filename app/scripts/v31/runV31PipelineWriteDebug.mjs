...
/**
 * Ortheon MVP Cut v3.1 — Live Pipeline Write Debug Runner
 *
 * Purpose:
 * Run the full live v3.1 pipeline and optionally persist the generated
 * V31PipelineResultPayload to assessments/{documentId}.v31Result.
 *
 * Bundle 15D rule:
 * - Debug-only runner.
    ...
    JSON.stringify(
      {
        ...
        ok: false,
        usage:
          "node scripts/v31/runV31PipelineWriteDebug.mjs --document-id=<assessmentDocId> [--assessment-id=<assessmentId>] [--write|--dry-run]",
      },
      null,
      2
    )
...
async function main() {
  ...
  const baseSnapshot = buildAssessmentSnapshotV31(SAMPLE_ASSESSMENT_V31);
